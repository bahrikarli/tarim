...
/** GİB alias — Türkçe büyük harf (İ) ve çift urn:mail önekini düzeltir. */
function edmAliasAsciiTemizle(alias) {
  let a = String(alias || '').trim();
  if (!a) return '';
  a = a.replace(/\u0130/g, 'I').replace(/\u0131/g, 'i');
  a = a.normalize('NFKD').replace(/[\u0300-\u036f]/g, '');
  while (/^urn:mail:/i.test(a)) {

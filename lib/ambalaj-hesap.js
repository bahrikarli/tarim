...
function planMaxAmbalaj(plan) {
  return Math.max(...(plan?.secim || []).map((s) => s.ambalajMiktari), 0);
}

/** Aynı fire: önce tek ambalaj satırı, az kutu; karışık planlar ancak onlar yoksa. */
function planDahaIyi(yeni, eski) {
  if (!eski) return true;
  if (yeni.fire < eski.fire - 1e-6) return true;
  if (yeni.fire > eski.fire + 1e-6) return false;
  const yTek = (yeni.secim || []).length === 1;
  const eTek = (eski.secim || []).length === 1;
  if (yTek && !eTek) return true;
  if (!yTek && eTek) return false;
  if (yeni.adetToplam !== eski.adetToplam) return yeni.adetToplam < eski.adetToplam;
  return planMaxAmbalaj(yeni) > planMaxAmbalaj(eski);
}
...
function boundedAramaPlan(variants, ihtiyac, maxAdet) {
  ...
  function dene(secim, basIdx, kalanAdet) {
    ...
    if (toplam >= ihtiyac - 1e-6) {
      ...
      if (planDahaIyi(plan, enIyi)) enIyi = plan;
      return;
    }
...
function ambalajOnerileri(ihtiyac, variants) {
  ...
  const azAtik = gecerli.reduce((a, b) => {
    if (a.fire < b.fire - 1e-6) return a;
    if (b.fire < a.fire - 1e-6) return b;
    return a.adetToplam <= b.adetToplam ? a : b;
  });
  const azKutu = gecerli.reduce((a, b) => {
    if (a.adetToplam < b.adetToplam) return a;
    if (b.adetToplam < a.adetToplam) return b;
    return a.fire <= b.fire ? a : b;
  });
  ...
  /** Varsayılan: tam bölünüyorsa tamUyum; yoksa en az fire (örn. 9,5 Lt → 2×5, 1×20 değil). */
  let enYakin = null;
  if (tamBolunmus && tamBolunmus.miktarToplam >= need - 1e-6) {
    enYakin = planKlon(tamBolunmus);
  } else if (azAtik) {
    enYakin = planKlon(azAtik);
  } else if (g1 && g1.miktarToplam >= need - 1e-6) {
    enYakin = planKlon(g1);
  } else {
    enYakin = planKlon(azKutu);
  }

  const enUzak = planKlon(enYakin);
  const tamDenk = !!tamUyum;
  const secimGerekli = false;

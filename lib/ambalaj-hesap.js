...
function ambalajOnerileri(ihtiyac, variants) {
  ...
  const planSirala = (a, b) => {
    if (Math.abs(a.fire - b.fire) > 1e-6) return a.fire - b.fire;
    const ambFark = planMaxAmbalaj(a) - planMaxAmbalaj(b);
    if (Math.abs(ambFark) > 1e-6) return ambFark;
    return a.adetToplam - b.adetToplam;
  };
  const gecerliKlon = gecerli.map(planKlon).sort(planSirala);
  /** İhtiyaca en yakın: en az fire; eşitlikte küçük ambalaj (örn. 1,5 Lt → 2×1 Lt). */
  const enYakin = planKlon(gecerliKlon[0]);
  /** İhtiyacı en az geçen: bir sonraki plan (örn. 1,5 Lt → 1×5 Lt). */
  let enUzak = gecerliKlon.find((p) => !planlarEsit(p, enYakin)) || null;

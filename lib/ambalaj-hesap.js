...
function planOzeti(secim, ihtiyac) {
  const adetToplam = secim.reduce((s, x) => s + x.adet, 0);
  const miktarToplam = secim.reduce((s, x) => s + x.adet * x.ambalajMiktari, 0);
  return {
    secim,
    adetToplam,
...
function ambalajOnerileri(ihtiyac, variants) {
  ...
  const adaylar = [];
  if (tamBolunmus) adaylar.push(tamBolunmus);
  for (const v of norm) {
    const t = tekBoyutPlan(v, need);
    if (t) adaylar.push(t);
  }
  const g1 = greedyPlan(norm, need, true);
  const g2 = greedyPlan(norm, need, false);
  const arama = boundedAramaPlan(norm, need, 24);
  if (g1 && !adaylar.includes(g1)) adaylar.push(g1);
  if (g2) adaylar.push(g2);
  if (arama) adaylar.push(arama);
  ...
  const enYakin = tamBolunmus || (tamUyum && tamUyum.fire < 1e-6 ? tamUyum : azAtik);
  const enUzak = gecerli.reduce((a, b) => (a.fire > b.fire + 1e-6 ? a : b.fire < a.fire - 1e-6 ? b : (a.adetToplam >= b.adetToplam ? a : b)));
  const tamDenk = !!tamUyum;
  const secimGerekli = !tamDenk && Math.abs((enYakin?.fire || 0) - (enUzak?.fire || 0)) > 1e-6;
...
module.exports = { ambalajOnerileri, planOzeti, planTamBuyuktenKucuk };
